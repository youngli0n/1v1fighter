...
def draw_stats_panel(player1, player2):
    ...
    section_padding = 10  # Padding between elements
    
    # Calculate element dimensions
    text_height = 20  # Height for text elements
    bar_height = 15   # Height for progress bars
    bar_width = section_width - 40  # Width for progress bars (with padding)
    ...
    def draw_player_stats(player, section_x, is_player1):
        ...
        speed_y = panel_y + section_padding
        shield_y = speed_y + text_height
        boosts_y = shield_y + text_height
        bar_y = boosts_y + text_height
        ...
        screen.blit(speed_surface, (section_x + section_padding, speed_y))
        ...
        screen.blit(shield_surface, (section_x + section_padding, shield_y))
        ...
        screen.blit(boosts_surface, (section_x + section_padding, boosts_y))
        ...
        pygame.draw.rect(screen, COLORS['progress_bar_bg'], 
                        (section_x + section_padding, bar_y, bar_width, bar_height))
        
        # Draw progress bar fill
        fill_width = int(bar_width * (progress / 100))
        pygame.draw.rect(screen, COLORS['progress_bar_fill'], 
                        (section_x + section_padding, bar_y, fill_width, bar_height))
        ...
        screen.blit(progress_surface, 
                   (section_x + section_padding + bar_width - text_width, bar_y))
...
def draw_victory_screen(winner, player1, player2):
    ...
    victory_rect = victory_surface.get_rect(center=(GAME_CONFIG['window_width_in_pixels'] // 2, 
                                                  GAME_CONFIG['window_height_in_pixels'] // 3))
    screen.blit(victory_surface, victory_rect)
    
    # Draw final stats
    stats_font = pygame.font.Font(None, 36)
    stats_y = victory_rect.bottom + 50
    
    # Player 1 stats
    p1_stats = [
        f"Player 1 Final Speed: {int(player1.get_total_speed_multiplier(time.time()) * 100)}%",
        f"Blocks: {len(player1.shield_boosts)}",
        f"Progress: {int(player1.get_progress())}%"
    ]
    
    # Player 2 stats
    p2_stats = [
        f"Player 2 Final Speed: {int(player2.get_total_speed_multiplier(time.time()) * 100)}%",
        f"Blocks: {len(player2.shield_boosts)}",
        f"Progress: {int(player2.get_progress())}%"
    ]
    ...
    restart_rect = restart_surface.get_rect(center=(GAME_CONFIG['window_width_in_pixels'] // 2, 
                                                  GAME_CONFIG['window_height_in_pixels'] * 3 // 4))
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if game_over:
                ...
                elif event.key == pygame.K_b:  # Player 1 shield
    ...
    if game_over:
        draw_victory_screen(winner, player1, player2)
